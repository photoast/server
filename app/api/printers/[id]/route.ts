...
export async function PATCH(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const { name, printMethod, email, epsonAuth, supportedSizes, borderCorrectionEnabled, shrinkPercent, verticalOffsetPx } = body

    const updates: any = {}
    if (name !== undefined) updates.name = name
    if (printMethod !== undefined) updates.printMethod = printMethod
    if (email !== undefined) updates.email = email
    if (epsonAuth !== undefined) updates.epsonAuth = epsonAuth
